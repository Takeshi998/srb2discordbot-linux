...
# Comando para abrir la consola del bot
#bot_cmd = ["gnome-terminal", "--", "python3", "srb2discordbot_linux_rc3.py"]

# Comando para abrir la consola del juego en modo dedicado
#game_cmd = ["gnome-terminal", "--", "./lsdl2srb2", "-dedicated"]

# Abre las ventanas de terminal del bot y del juego en procesos separados
#subprocess.Popen(bot_cmd)
#subprocess.Popen(game_cmd)

# Ejecuta el código principal en esta consola
#while True:
    # ...
    #pass
...
print(Fore.GREEN + Style.BRIGHT + "unofficial srb2discordbot_linux - Link your SRB2 server to your discord server!")
...
print(Fore.GREEN + Style.BRIGHT + "                      Modified By: T s u k o * ( S E R V A L ) *#2315          ")
print(Fore.GREEN + Style.BRIGHT + "          Youtube: https://www.youtube.com/@tsukograyrayzotakeshi8706          ")
print(Fore.GREEN + Style.BRIGHT + "          Serval Kart DX Discord Server: https://servalkartdx.carrd.co/        ")
print(Fore.GREEN + Style.BRIGHT + "                              Fixed By: Nua#1962                               ")
print(Fore.GREEN + Style.BRIGHT + "===============================================================================")
...
def json_config():
    ...
    while inp_token == "":
        ...
        print("Advertencia: Valor vacio.")
        print("El token es necesario para funcionar con Discord.")
        inp_token = input("Token of your discord bot:")
    inp_token = "\n\"token\": \""+inp_token+"\","
    ...
    while inp_channelpost == "":
        ...
        print("Advertencia: Valor vacio.")
        print("Es necesario el ID del canal donde se enviaran los mensajes del juego.")
        inp_channelpost = input("Message channel ID:")
    ...
    while inp_channelstatus == "":
         ...
         print("Advertencia: Valor vacio.")
         print("Es necesario el ID del canal donde se enviara el estado del juego.")
         inp_channelstatus = input("Information Channel ID:")
    ...
    if not inp_srb2app:
        print(Fore.YELLOW + "Warning: If you are hosting multiple SRB2 servers on this computer, enter a new name for the SRB2 binaty file. For example \"my_server1\". (This is to avoid conflicts with other servers)")
        inp_srb2app = input("The new name of the SRB2 EXE file:")
        if inp_srb2app == "":
            inp_srb2app = "lsdl2srb2"
        elif inp_srb2app != "":
            inp_srb2app = inp_srb2app.replace("", '')
            inp_srb2app = inp_srb2app+""
...
#If the file is damaged, then delete it and close the program
    ...
    if os.path.isfile("srb2discordbot/backups/config.json") == True:
        ...
        print("Error: Problema con la configuracion.")
        print("Archivo .json invalido, se procede a restaurar el backup/respaldo.")
    else:
        #messagebox.showerror("Error: Trouble with a config file.", "Invalid .json file configuration, it will be deleted.")
        print("Error: Problema con la configuracion.")
        print("Archivo .json invalido, el archivo sera eliminado. (Tarea pendiente: Evitar borrar el archivo de configuracion incluso si este es invalido)")
...
if os.path.isfile(config["srb2exe"]) == False:
    print("Debug: El archivo \"" + config["srb2exe"] + " no ha sido encontrado. Intentando con ruta local: \"lsdl2srb2\"")
    while os.path.isfile("lsdl2srb2") == False:
         #messagebox.showwarning("Warning", "lsdl2srb2 was not found.")
         print("Advertencia.")
         print("El archivo lsdl2srb2 no ha sido encontrado.")
         if os.path.isfile(config["srb2exe"]) == True:
             break
    if config["srb2exe"] != "lsdl2srb2" and os.path.isfile("lsdl2srb2") == True:
        shutil.copy(r'lsdl2srb2', config["srb2exe"])


#Parameters for binary srb2 file
...
if os.path.isfile("srb2discordbot/serverparameters/parameters.cfg") == False:
    ...
    print("Enter the parameters of the binary file. For example \"-dedicated -server -room 33 -file...\"")
...
def startserver():
    ...
    if isstarted == False:
        ...
        log_file.write("")
        log_file.close()
        print("Debug: Direccion del ejecutable: " + config["srb2exe"])
        print("Debug: Mapa seleccionado: " + mapsrb2)
        print("Debug: Parametros de inicio: " + parameters)
        if currentpcfg == None:
            processsrb2 = subprocess.Popen("./"+config["srb2exe"] + " " + mapsrb2 + " " + parameters, shell=True)
        else:
            processsrb2 = subprocess.Popen("./"+config["srb2exe"] + " " + mapsrb2 + " " + currentpcfg + " " + parameters, shell=True)
        isstarted = True
...
class MyClient(discord.Client):
    async def on_ready(self):
        ...
        #If the channel ID is not correct, display an error window and close the program
            ...
            print("Error: El ID del canal de log o mensajes es invalido.")
            print("Verifica el ID y realiza los cambios dentro de discordbot.json.")
        ...
        while True:
                ...
                if log_lens != 0:
                    ...
                    for line in log[log_lens:]:
                        if line.startswith("Entering main game loop..."):
                            ...
                            print(line[18:] + " no ha sido encontrado o no es valido.")
                            print("Encuentra el addon o remuevelo de srb2discordbot/serverparameters/parameters.cfg.")
                            print("Yo tampoco entiendo que quiere decir este mensaje -Noah")
            ...
            #If the channel ID is not correct, display an error window and close the program
                ...
                #Don't close the program on any error
                 ...
                 print("Valida que el bot tenga permisos de leer y enviar mensajes a los canales configurados.")
            except ConnectionResetError:
                if config["debug"] == True: #Display where the error occurred if debug is enabled
                    print("[" + now.strftime("%H:%M") + "]" + Fore.RED + 'Error:' + Style.RESET_ALL + ' failed to update status')
                ...
                if isdiscordborwork == True:
                    ...
                    if srbm != '':
                        ...
                        #If the channel ID is not correct, display an error window and close the program
                            ...
                            print("Error: El ID del canal de mensajes es invalido.")
                            print("Verifica el ID y realiza los cambios dentro de discordbot.json.")
                            exit()
                        except PermissionError:
                            if config["debug"] == True:
                    ...
                    if config["log_id"] != "None":
                        ...
                        if srblc != '':
                            ...
                            #If the channel ID is not correct, display an error window and close the program
                                ...
                                print("Error: El ID del canal de logs es invalido.")
                                print("Verifica el ID y realiza los cambios dentro de discordbot.json.")
                                exit()
                            except PermissionError:
                                if config["debug"] == True:
                    ...
                    if msgstats == None or (stateditcount >= 100 and msgstats != None):
                        ...
                        #If the channel ID is not correct, display an error window and close the program
                            ...
                            print("Error: El ID del canal de informacion es invalido.")
                            print("Verifica el ID y realiza los cambios dentro de discordbot.json.")
                            exit()
...
if isdiscordborwork == False:
    ...
    try:
        ...
        #If the token is not correct, display an error window and close the program
         ...
         print("Error: Token incorrecto o no valido.")
         print("Genera un nuevo token y remplazalo en el archivo discordbot.json")
         exit()
else:
    now = datetime.datetime.now()
    if config["debug"] == True:
